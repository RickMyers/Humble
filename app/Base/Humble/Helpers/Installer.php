...
SQL;
    ...
    protected function installSchema($source=false,$module=false)   {
        if ($source) {
            $source = str_replace('_','/',(''.$module->package.'/'.$source.'/'));
            if (is_dir($source)) {
    ...
    protected function storeStructure($prefix,$structure,$module=false) {
        ...
        if (is_dir(''.$package.'/'.str_replace('_','/',$images))) {
            $sourceDir      = ''.$package.'/'.str_replace('_','/',$images);
...
SQL;
    ...
    public function compileControllers($namespace=false) {
        ...
        $dest       = $source.'/Cache';
        $files      = \Humble::getHelper('humble/directory')->listDirectory(''.$source,false); //this is weird... where do I prepend 'Code' to the name already?
...
SQL;
    ...
    protected function installImages($prefix,$structure,$module=false)    {
        $images     = $structure->images->source;
        if (is_dir(''.$module->package.'/'.str_replace('_','/',$images))) {
            $sourceDir      = ''.$module->package.'/'.str_replace('_','/',$images);
            $destination    = "../images/".$this->namespace;
            @mkdir($destination,0775,true);
            $this->copyDirectory($sourceDir, $destination);
        }
...
SQL;
    ...
    public function generateLayoutSchema($package, $namespace, $source='',$entities=[],$force=false) {
        $destinationDirectory = str_replace('_','/',''.$package.'/'.$source);
...
FIELD;
                    ...
                    $json   = str_replace(['&&FORM&&','&&FIELDS&&','&&FORM_ID&&'],['default-'.$name.'-form',$fields,$stamp],file_get_contents('Base/Humble/lib/sample/component/DSL.json'));
            ...
            if ($xml = file_get_contents($source)) {
                ...
                //    if (isset($contents->events)) {
                 ...
                 //   }
                    ...
                    $install_file  = "".(string)$contents->module->package."\\".str_replace(["_","/"],["\\","\\"],(string)$contents->structure->models->source)."\\OnInstall.php";
                    $install_class = "".(string)$contents->module->package."\\".str_replace(["_","/"],["\\","\\"],(string)$contents->structure->models->source)."\\OnInstall";
