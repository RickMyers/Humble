...
class Updater extends Installer
{
    ...
    private function updateSchema($package,$namespace,$source)    {
        $dir = @dir($path = ''.$package.'/'.str_replace('_','/',$source));
    ...
    public function updateStructure($namespace,$structure,$module_data) {
        ...
        if (is_dir(''.$module_data->package.'/'.str_replace('_','/',$structure->images))) {
            $sourceDir      = ''.$module_data->package.'/'.str_replace('_','/',$structure->images);
          ...
          //  \Log::console('Starting Update Of: '.$source);
            if ($helper->isValidXML($xml = file_get_contents($source))) {
                ...
                foreach ($xml as $namespace => $contents) {
                    ...
                    $update_file  = "".(string)$contents->module->package."\\".str_replace(["_","/"],["\\","\\"],(string)$contents->structure->models->source)."\\OnUpdate.php";
                    $update_class = "".(string)$contents->module->package."\\".str_replace(["_","/"],["\\","\\"],(string)$contents->structure->models->source)."\\OnUpdate";
