<?php
namespace Base\Paradigm\Models;
use Humble;
use Log;
use Environment;
/**
...
class System extends Model
{
    ...
    public function templates() {
        ...
        foreach (Humble::getEntity('humble/modules')->setEnabled('Y')->fetch() as $module) {
            ...
            if (is_dir(''.$module['package'].'/'.$module['module'].'/web/app')) {
                $d = $dir->contents(''.$module['package'].'/'.$module['module'].'/web/app',true);
                foreach ($dir->contents(''.$module['package'].'/'.$module['module'].'/web/app',true) as $file) {
                    if (!is_dir($file)) {
                        $parts = explode('/',$file);
