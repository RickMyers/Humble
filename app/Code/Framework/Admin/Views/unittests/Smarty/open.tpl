    ...
    Landing.tests = (function (win) {
        ...
        return {
            ...
            load: function (window_id) {
                (new EasyAjax('/humble/unittests/load')).add('window_id',window_id).add('source',$('#unit-test-harness-source-'+window_id).val()).then((response) => {
                    $('#unit-test-harness-content-{$window_id}').html(response);
                }).post();
            },
            run: function (window_id) {
                (new EasyAjax('/humble/unittests/run')).add('window_id',window_id).add('source',$('#unit-test-harness-source-'+window_id).val()).then((response) => {
                    $('#unit-test-harness-content-{$window_id}').html(response);
                }).post();
            }
        }
