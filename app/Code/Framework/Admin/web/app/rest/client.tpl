<form name="rest_client_form" id="rest_client_form" onsubmit="return false">
    <div id="rest_client_header" class="bg-[#333333] pr-2 text-xl" style="color: ghostwhite; text-align: right">
        <br />
        Rest Client {{ window_id }}
        <br /><br />
    </div>
    <div id="rest_client_body" class="pl-6">
        <div id="rest_client_nav">
            
        </div>
        <div id="rest_client_parameters">
            ...
            <div class="w-1/5 inline-block">
                <div class="">
                    <select v-on:change="fetchControllers($event)" name="namespace" class="p-1 w-full">
                        <option value=""> </option>
                        <option v-for="(result,i) in namespaces" v-bind:value="result.namespace" :key="i" class=""> {{ result.namespace }} </option>
                    </select>
                </div>
                <div class="font-mono text-base tracking-wide pb-2">
                    Namespace
                </div>
            </div>
            <div class="w-1/5 inline-block">
                <div class="">
                    <select v-on:change="fetchControllerActions($event)" name="controller" class="p-1 w-full">
                        <option value=""> </option>
            ...
            <div class="w-1/5 inline-block">
                <div class="">
                    <select v-on:change="fetchActionParameters($event)" name="action" class="p-1 w-full">
                        <option value=""> </option>
            ...
            <table class="zebra-table"> 
                ...
                <tr v-for="(parameter,i) in parameters" :key="i" class="whitespace-nowrap w-full cursor-pointer zebra-row">
                    <td v-for="(data,field) in parameter" :key="field" class="w-32 text-center inline-block p-1 overflow-hidden font-mono text-sm text-cell" style="border: 1px solid transparent"> 
                        <input v-if="field === 'user_value'" type="text" v-bind:name="data" value="" class='p-1 w-full' style='background-color: lightcyan; border: 1px solid #333; border-radius: 2px'/>
        ...
        <div id="rest_client_request">
            <textarea v-bind:id="request_text_id" v-bind:value="request_text"></textarea>
        </div>
        <div id="rest_client_response"> 
            <textarea v-bind:id="request_text_id" v-bind:value="response_text"></textarea>
        </div>
        <div id="rest_client_render"> 
            <table class="zebra-table"> 
                <tr class="text-white bg-[#333333] whitespace-nowrap w-full">
                    <th v-for="(header,j) in response_headers" :key="jj" class="p-1 w-32 text-center inline-block overflow-hidden font-mono text-sm">
                        {{ header }} 
                    </th>
                </tr>
                <tr v-for="(fields,i) in response_fields" :key="i" class="whitespace-nowrap w-full cursor-pointer zebra-row">
                    <td v-for="(data,field) in fields" :key="field" class="w-32 text-center inline-block p-1 overflow-hidden font-mono text-sm text-cell" style="border: 1px solid transparent"> 
                        {{ data }} 
                    </td>
                </tr>
            </table>               
