...
<table style="width: 100%; height: 100%; border-spacing: 1px;">
    ...
    <tr>
        <td colspan="3" align="center" valign="middle">
            <form name="handler-error-form" id="handler-error-form-{$data.id}" onsubmit="return false">
                <input type="hidden" name="id" id="id_{$data.id}" value="{$data.id}" />
                <input type="hidden" name="window_id" id="window_id_{$data.id}" value="{$helper->getWindowId()}" />
                <table>
                    <tr><td>
                        Message: <input type="text" name="message" id="handler-error-{$data.id}" value="{if (isset($data.message))}{$data.message}{/if}" />
                        </td></tr>
                    <tr><td>
...
<script type="text/javascript">
    Form.intercept($('#handler-error-form-{$data.id}').get(),'{$data.id}','/paradigm/element/update',"{$helper->getWindowId()}");
</script>