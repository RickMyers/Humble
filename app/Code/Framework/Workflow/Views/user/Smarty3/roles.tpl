...
<table style="width: 100%; height: 100%; border-spacing: 1px;">
    ...
    <tr>
        <td colspan="3" align="center" valign="middle">
            <form name="config-roles-form" id="config-roles-form-{$data.id}" onsubmit="return false">
                <input type="hidden" name="id" id="id_{$data.id}" value="{$data.id}" />
                <input type="hidden" name="windowId" id="windowId_{$data.id}" value="{$helper->getWindowId()}" />
                <table>
                    <tr><td>
                            <select name="role_id" id="role_id_{$data.id}">
...
<script type="text/javascript">
    Form.intercept($('#config-roles-form-{$data.id}').get(),'{$data.id}','/paradigm/element/update',"{$helper->getWindowId()}");
</script>