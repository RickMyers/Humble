...
//	this.requiredColor	= "#ee3333";
	...
	this.packageForm = function ()
	{
    	return EasyEdits.packageForm(me);
	}
	this.fetch	= function (JSONsource)
	{
		if (JSONsource)
		{
			var ao = new EasyAjax(JSONsource);
			ao.setAsync(false);
			ao.setCallbackFunction(function () {
				me.editsJSON = ao.getResponse();
			});
			ao.post();
		}
	}
...
EasyEdits.execute	= function (easy){
	...
	if (easy.edits.form.widgets){
		for (var widget in easy.edits.form.widgets)	{
			switch (widget)
			{
				case	"calendar" 		:	widget = easy.edits.form.widgets[widget];
											...
											break;
				case	"timepicker"	:	widget = easy.edits.form.widgets[widget];
		...
		if ($E(easy.edits.fields[i].id) && easy.edits.fields[i].active)		{
            ...
            if (formField.onchange) {
				...
				if (easyField.type == "combo"){
					isCombo = true;
					/*if (!$E(formField.id+"_combo"))
					{
						//eventually migrate this code to event delegate handling, then the below will work...
						var nh = '<input type="text" name="'+easyField.id+'_combo" id="'+easyField.id+'_combo" />';
						form.innerHTML = form.innerHTML + nh;
					}*/
					...
					combo.onchange = function (evt)	{
						evt = (evt) ? evt : ((window.event) ? event : null);
						this.setAttribute("comboValue",this.value);
						$E(this.getAttribute("comboPair")).onchange(evt,true);
					}
				}
				formField.isCombo = isCombo;
				formField.onchange = function (evt,calledFromComboPair)	{
					evt = (evt) ? evt : ((window.event) ? event : null);
					var isCombo = (this.getAttribute("combo")=="yes");
					if (isCombo){
							...
							while (fCtr<fieldVal.length) {
								...
								if (tokens.indexOf(template[tCtr])!=-1)	{
									...
									switch (template[tCtr])	{
										case "#"	:   if (!(isNaN(parseInt(cc)) && (cc != ".") && (cc != "-"))) {
															results[results.length] = cc;
														}
														break;
										case "A"	:	if (!(!isNaN(parseInt(cc)))) {
															results[results.length] = cc;
														}
														break;
										case "^"	:   if (isNaN(parseInt(cc))) {
										...
										case "H"	:	cc=(cc+'').toUpperCase();
														if (hex.indexOf(cc)!=-1)
															results[results.length] = cc;
														break;
										case "*"	:	results[results.length] = cc;
														break;
										default		: 	break;
									}
									fCtr = fCtr+1;
				...
				if ((typeof(easyField.populator) != "undefined") && (easyField.populator)) {
					if (typeof(easyField.populator) == "string") {
						var targetUrl = easyField.populator;
				...
				if ((typeof(easyField.validator) != "undefined") && (easyField.validator))
				{
					if (typeof(easyField.validator)=="function")
						...
						formField.onblur = function () {
							var targetUrl = easyField.validator;
							var eao = new EasyAjax(targetUrl);
							eao.field = easyField.id;
							eao.setAsync(false);
							eao.then(function() {
				...
				if (window.getComputedStyle) {
					...
					easyField.baseFFy		= $E(formField.id).currentStyle["fontFamily"];
				}
				if (easyField.value) {
...
/* ------------------------------------------------ */
EasyEdits.getValue = function (form,field)
{
	var val	= "";
	try {
		switch (field.type.toLowerCase())
		{
			case "text"			:	val = field.value;
									break;
			case "hidden"		:	val = field.value;
									break;
			case "textarea"		:	val = field.value;
									break;
			case "checkbox" 	:	if (field.checked)
										 val = (field.value) ? field.value : "true";
									break;
			case "password"		:	val = field.value;
									break;
			case "radio"		:   var cbArray = form.elements[field.name]; var cbCtr = 0;
									...
									break;
			case "select-one"	:	val = (field.getAttribute("combo")) ? $E(field.id+"_combo").getAttribute("comboValue") : field[field.selectedIndex].value;
		...
		console.log("Get Field Value: "+ex);
	}
...
/* ------------------------------------------------ */
EasyEdits.packageEdits = function (easy,ao)
{
	for (var i=0; i<easy.edits.fields.length; i++)
	{
		if (easy.edits.fields[i].active)
		{
			var field = $E(easy.edits.fields[i].id); //TODO:
		}
	}
}
/* ------------------------------------------------ */
EasyEdits.send	= function (easy,URL)
{
	...
	if (target)
	{
		...
		if (easy.sendHandler)
			...
			ao.then(function () {alert(ao.getResponse()); });
		if (getMethod)
			ao.get();
		else
			ao.post();
		...
		if (easy.edits.fields[i].active)
		{
			...
			if (formField)
			{
				...
				if (fieldValues[formField.name])
					...
					fieldVal	= fieldValues[formField.name] = EasyEdits.getValue(form,formField);
				if (fieldVal)
...
/* ------------------------------------------------ */
EasyEdits.populateSelectBox = function (selectBox, contents)
{
	...
	if (selectBox)
	{
		...
		var oOption = null;
		var oGroup = null;
		for (var jk = 0; jk < contents.length; jk++) {
			if ((contents[jk].label) && (contents[jk].label != saveLabel))
			{
				if (oGroup)
					selectBox.appendChild(oGroup);
				oGroup = document.createElement('OPTGROUP');
				saveLabel = oGroup.label = contents[jk].label;
			}
			oOption = document.createElement('OPTION');
			oOption.value = contents[jk].value;
            if (contents[jk].style) {
                oOption.style = contents[jk].style;
            }
			if (isIE)
				oOption.innerText  = contents[jk].text;
			else
				oOption.text = contents[jk].text;
			if (oGroup)
	...
	else alert("I was supposed to populate a drop down box, but I didnt find it");
}
/* ------------------------------- */
...
EasyEdits.generateFormElementHTML	= function (edit)
{
	...
	{
		case "button"	:	elementHTML = "<input type='button' name='"+ edit.id +"' id='"+ edit.id +"' value='"+ edit.value +"' />";
							break;
		case "text" 	:	elementHTML = "<input type='text' name='"+edit.id+"' id='"+edit.id+"' value='"+ edit.value +"' />";
							break;
		case "select"	:	elementHTML = "<select combo='' name='"+edit.id+"' id='"+edit.id+"' ><option value=''></option></select>";
							break;
		case "combo"	:	elementHTML = "<select tabindex='99' onclick='EasyEdits.resetLastKey()' onfocus='EasyEdits.throwFocusAway(event)' onchange='EasyEdits.setComboBox(event)' combo='yes' name='"+edit.id+"' id='"+edit.id+"' ><option value=''></option></select>";
							elementHTML += '<input class="comboBox" type="text" name="'+edit.id+'_combo" id="'+edit.id+'_combo" style="border: 0px;" />';
							break;
		case "radio"	:	elementHTML = "<input type='radio' name='"+edit.group+"' id='"+edit.id+"' />";
							break;
		case "textarea"	:	elementHTML = "<textarea name='"+edit.id+"' id='"+edit.id+"' >"+ edit.value +"</textarea>";
							break;
		case "password"	:	elementHTML = "<input type='password' name='"+edit.id+"' id='"+edit.id+"' />";
							break;
		case "checkbox"	:	elementHTML = "<input type='checkbox' name='"+edit.id+"' id='"+edit.id+"' value='"+edit.value+"' />";
							break;
		default			:	break;
	}
	return elementHTML;
...
/* ------------------------------------------------ */
EasyEdits.setSelectBoxByValue	= function (selectBox,val)
{
	if (typeof(selectBox)=="string")
		selectBox = $E(selectBox);
	...
	{
		found = selectBox[ctr].selected = (val == selectBox[ctr].value);
		ctr++;
	}
}
/* ------------------------------------------------ */
...
EasyEdits.sum		= function (evt)
{
	...
	var sumField = $E(this.id).getAttribute("sumfield");
	var nodeList = [];
